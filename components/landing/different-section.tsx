...
export function DifferentSection() {
  return (
    <section className="py-16 lg:py-20 bg-gradient-to-b from-zinc-900 via-zinc-950 to-zinc-950 relative overflow-hidden">
      {/* Ambient glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[radial-gradient(circle,rgba(34,211,238,0.04)_0%,transparent_50%)] pointer-events-none" />

      <div className="mx-auto max-w-6xl px-6 relative">
        <div className="mx-auto max-w-2xl text-center mb-16">
          <h2 className="text-xs font-semibold text-cyan-400 uppercase tracking-[0.15em] mb-3">WHY STYLEUM</h2>
        ...
        <div className="mx-auto max-w-5xl grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-5">
          {differences.map((item, index) => (
            <ScrollReveal key={item.title} delay={index * 100}>
              <div
                className="relative bg-zinc-900 rounded-xl p-6 border border-zinc-800 transition-all duration-500 overflow-hidden group hover:shadow-[0_20px_50px_rgba(34,211,238,0.1)] hover:border-cyan-400/30 hover:-translate-y-2"
              >
                {/* Bottom accent gradient line */}
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-cyan-400 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500" />

                {/* Background glow on hover */}
                <div className="absolute inset-0 bg-gradient-to-t from-cyan-400/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

                <div className="relative z-10">
                  {/* Icon with glow effect on hover */}
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-cyan-400 text-zinc-950 mb-4 transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_25px_rgba(34,211,238,0.4)]">
        ...
        <ScrollReveal delay={300} visibleClassName="animate-slide-tilt-in">
          <div className="mt-8 max-w-2xl mx-auto aspect-[16/9] bg-gradient-to-br from-zinc-900 to-cyan-400/10 rounded-2xl flex items-center justify-center border border-zinc-800 hover:border-cyan-400/30 hover:shadow-[0_20px_40px_rgba(34,211,238,0.08)] transition-all duration-500">
